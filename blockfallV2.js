...
(function () {
  ...
  // --- Bot State ---
  let botRunning = localStorage.getItem('bfBotAutoOn') !== 'false'; // Defaults to true
  let autoBuyEnabled = localStorage.getItem('bfAutoBuy') === 'true'; // Defaults to false
  let botLogs = [];
  ...
  function updateBotUI() {
    ...
    if (!privWallet || !privVault) {
      ...
      html += `
        ...
        <div class="bf-consume-row">
          <span class="bf-label" style="color:${autoBuyEnabled ? '#00d4ff' : '#555'}">AUTO BUY</span>
          <button class="bf-toggle ${autoBuyEnabled ? 'on' : ''}" id="bf-autobuy-toggle"></button>
        </div>
       `;
    }
    ...
    document.getElementById('bf-autobuy-toggle')?.addEventListener('click', () => {
      autoBuyEnabled = !autoBuyEnabled;
      localStorage.setItem('bfAutoBuy', autoBuyEnabled);
      addBotLog(`Auto Buy: ${autoBuyEnabled ? 'ON' : 'OFF'}`);
      updateBotUI();
    });
  }
      ...
      if (walletBal.amount === 0n) {
        ...
        if (balanceConfirmed) {
          ...
          if (autoBuyEnabled) {
            ...
            addBotLog('[STEP 5] Auto buy OFF, skipping...');
            console.log('[BF Bot] [STEP 5] Auto buy OFF, skipping...');
          }
          ====== END AUTO BUY ====== */
          addBotLog('[STEP 5] Auto buy DISABLED (commented out)');
          console.log('[BF Bot] [STEP 5] Auto buy DISABLED');
        }
  ...
  //   return body;
  // }
  // ====== END AUTO BUY FUNCTIONS ======

  /* ====== AUTO BUY - DISABLED FOR NOW ======
  // --- Auto market buy: tier 3-5, floor 50-100 ---
  async function doMarketBuyCycle(ctx) {
    const { connection, wallet } = ctx;
    try {
      addBotLog(`[BUY] Scanning tier 3-5, floor 50-100...`);
      ...
      // Loop through listings and try to buy each one
      // Skip first listing (often sold instantly), start from index 1
      for (let i = 1; i < listings.length; i++) {
        if (!botRunning) return;
        const listing = listings[i];
        if (listing.sellerWallet === myWallet) continue;
        ...
        const listingId = listing.ids[0];
        const price = listing.price;
        const sellerHasAta = !!(await connection.getAccountInfo(sellerAta, 'confirmed'));
        if (!sellerHasAta) {
          console.log(`[BF Bot] [BUY] Skip ${listing.sellerHandle || sellerWallet.toBase58().slice(0, 6)}: no ATA`);
          continue;
        }
        console.log(`[BF Bot] [BUY] Trying: ${listing.sellerHandle || sellerWallet.toBase58().slice(0, 6)} @ ${price}`);
        ...
        try {
          ...
          transferView.setUint8(0, 12);
          transferView.setBigUint64(1, transferAmount, true);
          transferView.setUint8(9, 6);
          ...
          // BurnChecked - burn 1 BLOCK
          const burnBuf = new ArrayBuffer(10);
          const burnView = new DataView(burnBuf);
          burnView.setUint8(0, 15);
          burnView.setBigUint64(1, 1000000n, true);
          burnView.setUint8(9, 6);
          ...
          if (!confirmed) { console.log(`[BF Bot] [BUY] TX not confirmed, trying next...`); continue; }

          // Call buy endpoint
          const buyRes = await originalFetch('https://blockfall.io/api/market/buy', {
            method: 'POST', credentials: 'include',
            headers: { 'accept': '*' + '/' + '*', 'content-type': 'application/json', 'sec-fetch-dest': 'empty', 'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin' },
            body: JSON.stringify({ listingId, signature: txSignature })
          });
          const buyBody = await buyRes.json().catch(() => null);
          console.log(`[BF Bot] [BUY] buy response: ${buyRes.status} ${JSON.stringify(buyBody).slice(0, 200)}`);
          ...
          const errMsg = buyBody?.error || `HTTP ${buyRes.status}`;
          console.log(`[BF Bot] [BUY] Failed ${itemSlug}: ${errMsg}, trying next...`);
          continue;

        } catch (e) {
          console.log(`[BF Bot] [BUY] Error ${itemSlug}: ${e.message}, trying next...`);
          continue;
        }
      }

      addBotLog('[BUY] No item purchased this cycle.');
      console.log('[BF Bot] [BUY] No item purchased this cycle.');
