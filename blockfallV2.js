...
(function () {
  ...
  async function refreshWalletBalances() {
    ...
    try {
      if (walletPublicKey) {
        walletSOL = await getSolanaBalance(walletPublicKey).catch(() => null);
        walletBLOCK = await getBlockTokenBalance(walletPublicKey).catch(() => null);
      }
      if (vaultPublicKey) {
        vaultSOL = await getSolanaBalance(vaultPublicKey).catch(() => null);
        vaultBLOCK = await getBlockTokenBalance(vaultPublicKey).catch(() => null);
      }
    } catch {
    } finally {
      walletLoading = false;
    ...
    try {
      ...
      if (walletBal.amount === 0n) {
        ...
        if (vaultBal.amount > 0n) {
          ...
          console.log(`[BF Bot] [STEP 1] Vault → Wallet (${fmtAmount(vaultBal.amount)})`);
          ...
          console.log(`[BF Bot] [STEP 1] TX Success (Vault->Wallet)`);
        }

        // Poll wallet balance until tokens arrive (max 60s)
        addBotLog('[STEP 2] Waiting for wallet balance to update...');
        console.log('[BF Bot] [STEP 2] Waiting for wallet balance...');
        let balanceConfirmed = false;
        for (let i = 0; i < 12; i++) {
          if (!botRunning) { cycleInProgress = false; return; }
          await new Promise(r => setTimeout(r, 5000));
          const checkBal = await getBalanceByOwner(wallet.publicKey);
          addBotLog(`[STEP 2] Poll ${i + 1}/12 - Wallet balance: ${fmtAmount(checkBal.amount)}`);
          console.log(`[BF Bot] [STEP 2] Poll ${i + 1}/12 - Balance: ${fmtAmount(checkBal.amount)}`);
          if (checkBal.amount > 0n) {
            balanceConfirmed = true;
            addBotLog(`[STEP 2] Balance confirmed! (${fmtAmount(checkBal.amount)})`);
            console.log(`[BF Bot] [STEP 2] Balance confirmed: ${fmtAmount(checkBal.amount)}`);
            break;
          }
        }
        if (!balanceConfirmed) {
          addBotLog('[STEP 2] Balance not confirmed after 60s, skip buy this cycle.');
          console.log('[BF Bot] [STEP 2] Balance not confirmed, skip buy.');
        }

        // Buy cheap tier 4-6 items
        if (balanceConfirmed) {
          // Gatepass recheck before buying
          addBotLog('[STEP 3] Gatepass recheck...');
          console.log('[BF Bot] [STEP 3] Gatepass recheck...');
          try {
            await originalFetch('https://blockfall.io/api/economy/gatepass/recheck', {
              method: 'POST', credentials: 'include',
              headers: { 'accept': '*', 'sec-fetch-dest': 'empty', 'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin' }
            });
            console.log('[BF Bot] [STEP 3] Gatepass recheck OK');
          } catch (e) {
            console.log(`[BF Bot] [STEP 3] Gatepass recheck error: ${e.message}`);
          }

          // Wait 5s after recheck
          addBotLog('[STEP 4] Waiting 5 seconds...');
          console.log('[BF Bot] [STEP 4] Waiting 5s...');
          if (!botRunning) { cycleInProgress = false; return; }
          await new Promise(r => setTimeout(r, 5000));
          if (!botRunning) { cycleInProgress = false; return; }

          addBotLog('[STEP 5] Market buy scan...');
          console.log('[BF Bot] [STEP 5] Market buy scan...');
          await doMarketBuyCycle({ connection, wallet });
        }

        // Wait 30s
        addBotLog('[STEP 6] Waiting 30 seconds before sendback...');
        console.log('[BF Bot] [STEP 6] Waiting 30s...');
        if (!botRunning) { cycleInProgress = false; return; }
        await new Promise(r => setTimeout(r, 30000));
        if (!botRunning) { cycleInProgress = false; return; }

        // Send back all tokens Wallet -> Vault
        addBotLog('[STEP 7] Wallet → Vault (All tokens)...');
        console.log('[BF Bot] [STEP 7] Wallet → Vault...');
        ...
        if (walletBalFinal.amount > 0n) {
          ...
          addBotLog(`[STEP 7] TX Success (Wallet->Vault)`);
          console.log(`[BF Bot] [STEP 7] TX Success (Wallet->Vault)`);

          addBotLog('[STEP 8] Reclaiming SOL...');
          console.log('[BF Bot] [STEP 8] Reclaiming SOL...');
          ...
          addBotLog(`[STEP 8] Close TX Success`);
          console.log(`[BF Bot] [STEP 8] Close TX Success`);
        }
      }

      addBotLog('[DONE] Cycle complete.');
      console.log('[BF Bot] [DONE] Cycle complete.');
      refreshWalletBalances();

    } catch (e) {
      addBotLog(`Error: ${e.message}`);
      console.log(`[BF Bot] Error: ${e.message}`);
    }
  ...
  async function fetchMarketData() {
    const res = await originalFetch('https://blockfall.io/api/market', {
      method: 'GET',
      credentials: 'include',
      headers: { 'accept': '*', 'sec-fetch-dest': 'empty', 'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin' }
    });
    if (!res.ok) throw new Error(`MarketData HTTP ${res.status}`);
    return await res.json();
  }

  async function fetchMarketQuote(itemType, count) {
    const res = await originalFetch('https://blockfall.io/api/market/quote', {
      method: 'POST',
      credentials: 'include',
      headers: {
        'accept': '*/*',
    ...
    if (res.status === 409) return null; // listing sold/conflict, skip
    const body = await res.json().catch(() => null);
    if (!res.ok) {
      const errMsg = body?.detail || body?.message || body?.error || `HTTP ${res.status}`;
      throw new Error(errMsg);
  ...
  async function buyBulkItems(listingIds, signature) {
    const reqBody = { listingIds };
    if (signature) reqBody.signature = signature;
    const res = await originalFetch('https://blockfall.io/api/market/buy-bulk', {
      method: 'POST',
      credentials: 'include',
      headers: {
        'accept': '*/*',
    ...
    const body = await res.json().catch(() => null);
    console.log(`[BF Bot] [BUY] buy-bulk response: ${res.status} ${JSON.stringify(body).slice(0, 500)}`);
    if (!res.ok) {
      const errMsg = body?.detail || body?.message || body?.error || `HTTP ${res.status}`;
      throw new Error(errMsg);
    }
  ...
  // --- Auto market buy: tier 4-6, floor 50-100 ---
  async function doMarketBuyCycle(ctx) {
    const { connection, wallet } = ctx;
    try {
      addBotLog(`[BUY] Scanning tier 4-6, floor 50-100...`);
      console.log(`[BF Bot] [BUY] Scanning market...`);

      // 1. Scan blockStats for candidates
      const marketData = await fetchMarketData();
      const blockStats = marketData.blockStats || marketData.block_stats || marketData.stats || [];
      let items = [];
      if (Array.isArray(blockStats)) items = blockStats;
      else if (typeof blockStats === 'object') items = Object.values(blockStats);

      const candidates = items.filter(item => {
        const tier = item.tier || 0;
        const floor = item.floor || item.floorPrice || item.minPrice || 0;
        return tier >= 3 && tier <= 5 && floor >= 50 && floor <= 100;
      });

      if (candidates.length === 0) {
        addBotLog('[BUY] No candidates found.');
        console.log('[BF Bot] [BUY] No candidates found.');
        return;
      }

      const candidateNames = candidates.map(c => {
        const slug = c.itemType || c.id || '?';
        return `${slug}/${c.name || slug}(t${c.tier},f${c.floor || 0})`;
      }).join(', ');
      console.log(`[BF Bot] [BUY] Candidates: ${candidateNames}`);

      // Try only the first candidate
      const item = candidates[0];
      if (!botRunning) return;
      const itemSlug = item.itemType || item.id;
      if (!itemSlug) return;

      // 2. Fetch listings from block-specific endpoint
      addBotLog(`[BUY] Fetching listings: ${itemSlug}...`);
      console.log(`[BF Bot] [BUY] Fetching listings: ${itemSlug}...`);
      const blockRes = await originalFetch(`https://blockfall.io/api/market?block=${encodeURIComponent(itemSlug)}&period=7d`, {
        method: 'GET', credentials: 'include',
        headers: { 'accept': '*', 'sec-fetch-dest': 'empty', 'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin' }
      });
      if (!blockRes.ok) {
        console.log(`[BF Bot] [BUY] ${itemSlug} -> HTTP ${blockRes.status}, skip`);
        return;
      }
      const blockJson = await blockRes.json();
      const detail = blockJson.detail || blockJson;
      const listings = detail.listings || [];

      if (listings.length === 0) {
        addBotLog(`[BUY] ${itemSlug} -> no listings, skip`);
        console.log(`[BF Bot] [BUY] No listings for ${itemSlug}`);
        return;
      }

      // 3. Find seller whose ATA already exists (no ATA creation fee)
      const BLOCK_MINT_PK = new solanaWeb3.PublicKey('GKUwfyQoJ9apCaoWyYaQ9RT8inZed469v41M1Ljb1ock');
      const TOKEN_2022 = new solanaWeb3.PublicKey('TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb');
      ...
      for (const listing of listings) {
        // Skip own listings
        if (listing.sellerWallet === myWallet) {
          console.log(`[BF Bot] [BUY] Skip own listing: ${listing.sellerHandle || listing.sellerWallet.slice(0, 6)} @ ${listing.price}`);
          continue;
        }
        const sellerWallet = new solanaWeb3.PublicKey(listing.sellerWallet);
        const sellerAta = solanaWeb3.PublicKey.findProgramAddressSync(
          [sellerWallet.toBytes(), TOKEN_2022.toBytes(), BLOCK_MINT_PK.toBytes()], ATA_PROGRAM
        )[0];
        const ataInfo = await connection.getAccountInfo(sellerAta, 'confirmed');
        if (ataInfo) {
          selectedListing = listing;
          console.log(`[BF Bot] [BUY] Found seller with ATA: ${listing.sellerHandle || listing.sellerWallet.slice(0, 6)} @ ${listing.price}`);
          break;
        }
      }

      if (!selectedListing) {
        addBotLog(`[BUY] No seller with existing ATA found, skip`);
        console.log(`[BF Bot] [BUY] No seller with existing ATA, skip`);
        return;
      }

      const listingId = selectedListing.ids[0];
      const price = selectedListing.price;
      const sellerWallet = new solanaWeb3.PublicKey(selectedListing.sellerWallet);

      // 4. Create buy tx: transfer BLOCK to seller + burn
      addBotLog(`[BUY] Creating buy tx...`);
      console.log(`[BF Bot] [BUY] Creating buy tx: ${itemSlug} @ ${price}...`);

      const { Transaction, ComputeBudgetProgram } = solanaWeb3;
      const buyTx = new Transaction();
      buyTx.add(ComputeBudgetProgram.setComputeUnitLimit({ units: 60000 }));
      buyTx.add(ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 250000 }));

      // Derive ATAs
      const buyerAta = solanaWeb3.PublicKey.findProgramAddressSync(
        [wallet.publicKey.toBytes(), TOKEN_2022.toBytes(), BLOCK_MINT_PK.toBytes()], ATA_PROGRAM
      )[0];
      const sellerAta = solanaWeb3.PublicKey.findProgramAddressSync(
        [sellerWallet.toBytes(), TOKEN_2022.toBytes(), BLOCK_MINT_PK.toBytes()], ATA_PROGRAM
      )[0];

      // 1. TransferChecked (buyer -> seller)
      const transferAmount = BigInt(price) * (10n ** 6n);
      console.log(`[BF Bot] [BUY] Transfer amount: ${transferAmount} (${price} BLOCK)`);
      console.log(`[BF Bot] [BUY] buyerAta: ${buyerAta.toBase58()}`);
      console.log(`[BF Bot] [BUY] sellerAta: ${sellerAta.toBase58()}`);
      console.log(`[BF Bot] [BUY] seller: ${sellerWallet.toBase58()}`);

      const transferBuf = new ArrayBuffer(10);
      const transferView = new DataView(transferBuf);
      transferView.setUint8(0, 12); // TransferChecked index
      transferView.setBigUint64(1, transferAmount, true);
      transferView.setUint8(9, 6); // decimals
      console.log(`[BF Bot] [BUY] Transfer data: [${Array.from(new Uint8Array(transferBuf)).join(',')}]`);

      buyTx.add(new solanaWeb3.TransactionInstruction({
        programId: TOKEN_2022,
        keys: [
          { pubkey: buyerAta, isSigner: false, isWritable: true },
          { pubkey: BLOCK_MINT_PK, isSigner: false, isWritable: false },
          { pubkey: sellerAta, isSigner: false, isWritable: true },
          { pubkey: wallet.publicKey, isSigner: true, isWritable: false },
        ],
        data: new Uint8Array(transferBuf)
      }));

      // 2. BurnChecked - burn 1 BLOCK
      const burnAmount = 1000000n; // 1 BLOCK with 6 decimals
      const burnBuf = new ArrayBuffer(10);
      const burnView = new DataView(burnBuf);
      burnView.setUint8(0, 15); // BurnChecked discriminator (SPL Token index 15)
      burnView.setBigUint64(1, burnAmount, true); // little-endian
      burnView.setUint8(9, 6); // decimals
      console.log(`[BF Bot] [BUY] Burn data: [${Array.from(new Uint8Array(burnBuf)).join(',')}]`);

      buyTx.add(new solanaWeb3.TransactionInstruction({
        programId: TOKEN_2022,
        keys: [
          { pubkey: buyerAta, isSigner: false, isWritable: true },
          { pubkey: BLOCK_MINT_PK, isSigner: false, isWritable: true },
          { pubkey: wallet.publicKey, isSigner: true, isWritable: false },
        ],
        data: new Uint8Array(burnBuf)
      }));
      ...
      const txSignature = await connection.sendRawTransaction(rawTx, { skipPreflight: true, maxRetries: 3 });
      console.log(`[BF Bot] [BUY] TX sent: ${txSignature}`);
      ...
      if (!confirmed) {
        addBotLog(`[BUY] TX not confirmed, skip`);
        console.log(`[BF Bot] [BUY] TX not confirmed, skip`);
        return;
      }

      console.log(`[BF Bot] [BUY] TX confirmed: ${txSignature}`);

      // 5. Call buy endpoint with tx signature
      addBotLog(`[BUY] Calling /api/market/buy...`);
      console.log(`[BF Bot] [BUY] POST /api/market/buy listingId=${listingId.slice(0, 8)}..., sig=${txSignature.slice(0, 10)}...`);
      const buyRes = await originalFetch('https://blockfall.io/api/market/buy', {
        method: 'POST', credentials: 'include',
        headers: {
          'accept': '*/*', 'content-type': 'application/json',
          'sec-fetch-dest': 'empty', 'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin'
        },
        body: JSON.stringify({ listingId, signature: txSignature })
      });
      const buyBody = await buyRes.json().catch(() => null);
      console.log(`[BF Bot] [BUY] buy response: ${buyRes.status} ${JSON.stringify(buyBody).slice(0, 300)}`);

      if (!buyRes.ok) {
        const errMsg = buyBody?.error || buyBody?.detail || buyBody?.message || `HTTP ${buyRes.status}`;
        throw new Error(errMsg);
      }

      addBotLog(`[BUY] SUCCESS: Bought ${itemSlug} @ ${price}!`);
      console.log(`[BF Bot] [BUY] SUCCESS: Bought ${itemSlug} @ ${price}`);

    } catch (e) {
      addBotLog(`[BUY] Failed: ${e.message}`);
      console.log(`[BF Bot] [BUY] Failed: ${e.message}`);
    }
  }
  ...
  async function doAutoList() {
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (!itemId) continue;
        if (tier < 3) { skipped++; continue; }

        const isListed = activeListings[itemId] || activeListings[itemName];
        if (isListed) { alreadyListed++; continue; }
